...
export default {
    ...
    mounted(){
        ...
        this.$store.commit('setMinInArray',2)
        if(this.survey[this.modulename][this.fieldname][this.pos] != '')
        {
            let formattedSelect = ''
            let valSelect = ''
            for(let key in this.items){

                if(this.items[key].includes(this.survey[this.modulename][this.fieldname][this.pos])){
                    valSelect = key
            ...
            this.selected2 = this.survey[this.modulename][this.fieldname][this.pos]
    ...
    methods: {
        ...
        handleClickProject: function(val){
            ...
            this.$store.commit('updateSyncDataPos',params)
        }
        
    },
    props: ['pos','heading', 'withOthers','modulename','fieldname', 'items','defaultoption','format','defaultoption2'],
    components: { CheckboxItem , DropDownitem},
    watch: {
        selected: function(newV,oldV)
        {
            if(this.survey[this.modulename][this.fieldname][this.pos] != '' && !this.resultOptions.includes(this.selected2)){
